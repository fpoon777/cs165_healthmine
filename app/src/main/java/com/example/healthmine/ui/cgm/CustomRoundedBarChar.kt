...
class CustomRoundedBarChart : BarChart {
    ...
    private fun readRadiusAttr(context: Context, attrs: AttributeSet) {
        ...
        try {
            setRadius(a.getDimensionPixelSize(R.styleable.RoundedBarChart_radius, 50))
        } finally {
            a.recycle()
        }
    }
    ...
    private inner class RoundedBarChartRenderer internal constructor(
        ...
        override fun drawHighlighted(c: Canvas, indices: Array<Highlight?>) {
            ...
            for (high in indices) {
                val set = high?.let { barData.getDataSetByIndex(it.getDataSetIndex()) }
